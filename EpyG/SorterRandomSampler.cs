...
namespace EpyG
{
    public static class SorterRandomSampler
    {
        ...
        public static SorterSamplerResults SorterSampler(
                PhenotyperSorterType genomeSorterType,
                int keyCount,
                int switchCount,
                int histogramMin,
                int histogramMax,
            ...
            )
        {
            ...
            foreach (var results in RandomSorterTests(genomeSorterType, seed, keyCount, switchCount).Take(repCount))
            {
        ...
        static IEnumerable<ISorter> RandomSorters(int seed, int keyCount, int switchLength)
        {
            ...
            while (true)
            {
                yield break; // return rando.ToSorter2(keyCount, switchLength, randoK.NextGuid());
            }
        }


        static IEnumerable<IEnumerable<ISorterEval>> RandomSorterTests(
            PhenotyperSorterType genomeSorterType,
            int seed, 
            int keyCount,
            int switchLength
            )
        {
            ...
            {
                yield break; // return chunk.AsParallel().Select(s => s.Sort(testSet));
            }
